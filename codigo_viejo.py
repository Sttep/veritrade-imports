...
CODES: dict[str, tuple[str, str]] = {
    ...
    "KM":           ("kilometraje",      "num"),
    "KLM":          ("kilometraje",      "num"),   # variante real en datos
    "CLASIFICACION":("clasificacion",    "text"),
    "CAJA":         ("caja",             "text"),
    # Variantes reales encontradas en datos
    "VE":           ("version",          "text"),   # variante de VERSION:
    "CH/VIN":       ("vin",              "text"),   # combo chasis/vin
    # Terminadores — no los guardamos pero el regex para aquí
    # (evitan que el campo anterior capture valores de códigos desconocidos)
...
def _convertir(valor: str, tipo: str):
    ...
    if tipo == "power":
        ...
        try:
            ...
            return None
    return valor.strip(" ,;") or None  # text
...
def parsear_descripcion(texto: str) -> dict:
    ...
    for match in CODE_PATTERN.finditer(texto):
        ...
        if code in CODES:
            ...
            if campo != "_" and campo not in resultado:  # "_" = terminador, no guardar
...
def debe_excluir(row: dict, cfg: Config) -> tuple[bool, str]:
    ...
    # ── Categoría declarada en la descripción (L5, L6 = excluir siempre) ──────
    ...
    # ── Texto libre con señales de exclusión ──────────────────────────────────
    for termino in cfg.excluir_set:
        if len(termino) > 4 and termino in desc:   # solo términos largos para evitar falsos positivos
...
def _extraer_posicional(desc: str, cfg: Config) -> dict:
    """
    Maneja dos formatos sin etiquetas explícitas MARCA:/MODELO:

    Formato CSV:   N1,KAMA,KMC1030BEVA240WK VE:SIN VER,2023...
    Formato libre: N2 CHASIS CABINADO HINO DUTRO DOBLE CABINA AÑO 2023...

    Retorna dict parcial que se fusiona con el resultado del parseo por códigos.
    Solo completa campos que el parseo no encontró — nunca los sobreescribe.
    """
    resultado: dict = {}
    texto_up = str(desc).upper().strip()

    # Quitar prefijo de categoría ATU: N1, N2, N3, M1, L5, etc.
    texto_sin_cat = re.sub(r"^(?:N[123]|M[123]|L[12456])\s*[,\s]\s*", "", texto_up).strip()
    if not texto_sin_cat:
        return resultado

    # Encontrar dónde empieza el primer código estructurado (CA:, FR:, etc.)
    m_inicio = CODE_PATTERN.search(texto_sin_cat)
    cabecera = texto_sin_cat[:m_inicio.start()].strip() if m_inicio else texto_sin_cat

    # Extraer marca y modelo de la cabecera
    if "," in cabecera:
        # Formato CSV: primer token = marca, segundo = modelo
        tokens = [t.strip(" ,.") for t in cabecera.split(",") if t.strip(" ,.")]
        if tokens:
            resultado["_marca_pos"] = tokens[0]
        if len(tokens) >= 2:
            resultado["_modelo_pos"] = tokens[1].split()[0]  # primera palabra
    else:
        # Texto libre: buscar marca conocida en la cabecera
        m = cfg.brand_re.search(cabecera)
        if m:
            resultado["_marca_pos"] = m.group(0)
            # Modelo = primera palabra después de la marca que no sea descriptora
    ...
    # Carrocería libre: buscar keywords en la cabecera completa
    CARROCERIA_KW = {
...
def procesar_fila(row: pd.Series, cfg: Config) -> dict:
    ...
    # ── Parseo por códigos (CA:, FR:, MARCA:, etc.) ───────────────────────────
    extraido = parsear_descripcion(desc)

    # ── Extracción posicional (fallback para formatos sin etiquetas) ──────────
    posicional = _extraer_posicional(desc, cfg)

    # ── Marca ─────────────────────────────────────────────────────────────────
    # Prioridad: MARCA: explícito → posicional → regex en texto completo
    ...
    # ── Carrocería ────────────────────────────────────────────────────────────
    # Prioridad: CA: explícito → keyword en cabecera libre
...
# ── Skip si ya fue procesado ──────────────────────────────────────────────
    ...
    filas    = df_raw.to_dict("records")        # ← evita 71K objetos Series
    total    = len(filas)
 ...
 # ── Escribir Excel ────────────────────────────────────────────────────────
    ...
    if n_total <= 50_000:
        # Volumen normal → openpyxl con formato completo
        ...
        # Volumen grande → xlsxwriter en modo streaming (sin cargar en RAM)
        ...
        with pd.ExcelWriter(
            ...
            if not df_exc.empty:
                df_exc.to_excel(writer, sheet_name="_excluidos",    index=False)
            df_log.to_excel(writer,     sheet_name="_log",          index=False)
        # xlsxwriter streaming no soporta post-formato, pero el archivo queda limpio
