...
auditor_vs_app.py
─────────────────
Audita el reporte anual de la app (referencia externa) y, si existen outputs
del pipeline propio, los compara contra esa referencia.
...
MODO 2 — Con pipeline (cuando existen outputs/_estructurado.xlsx o _normalizado.xlsx):
...
Inputs:
  ...
  outputs/*_estructurado.xlsx     ← pipeline Fase 1 (opcional)
  outputs/*_normalizado.xlsx      ← pipeline Fase 2 (opcional)
...
def cargar_pipeline(años: list[int]) -> pd.DataFrame | None:
    patrones = ["*_normalizado.xlsx", "*_estructurado.xlsx"]
    archivos = []
    for pat in patrones:
        archivos += sorted(PIPELINE_DIR.glob(pat))
    # excluir el propio output de auditoría
    ...
    for path in archivos:
        try:
            ...
            hoja = next(
                (s for s in ["normalizado_llm", "estructurado"] if s in xl.sheet_names),
                xl.sheet_names[0],
            )
            df = pd.read_excel(path, sheet_name=hoja, dtype=str)
            df["_source"] = path.name
            frames.append(df)
            print(f"    ✅ Pipeline: {path.name} ({len(df)} filas, hoja '{hoja}')")
        except Exception as e:
...
def comparar_pipeline(
    ...
    col_marca = next(
        (c for c in ["marca_norm", "marca", "marca_raw"] if c in df_pip.columns), None
    )
    col_año = next(
        (c for c in ["año_dua", "anio_dua", "año", "anio"] if c in df_pip.columns), None
    )
    ...
    pivot_ref["app_total"] = pivot_ref[f"app_{años[-1]}"].fillna(0)  # último año o total

    df_comp = pivot_ref.join(pivot_pip, how="outer").fillna(0).reset_index()
    df_comp.rename(columns={"index": "marca", "app_TOTAL_GENERAL": "app_total"}, inplace=True)

    if "app_total" not in df_comp.columns:
        df_comp["app_total"] = df_comp[[f"app_{a}" for a in años if f"app_{a}" in df_comp.columns]].sum(axis=1)

    df_comp["delta"] = df_comp["pip_total"] - df_comp["app_total"]
    df_comp["pct_desvio"] = df_comp.apply(
        lambda r: 0 if r["app_total"] == 0 else abs(r["delta"]) / r["app_total"] * 100, axis=1
    )
